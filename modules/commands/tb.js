...
module.exports.config = {
    ...
    hasPermission: 2,
    credits: "Tkhanh rác",
    description: "Công cụ giúp admin bot cho thuê bot",
    commandCategory: "Admin",
    usages: "[key/check/clear/list/del/info]",
    cooldowns: 5,
    usePrefix: true,
    dependencies: {
        "crypto": "",
...
https://graph.facebook.com/${info.id}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`), await streamURL(threadInfo.imageSrc)]
       ...
       case 'del': {
			...
			send(`✅ Đã xóa data thuê bot của nhóm thành công`)
			await save()
			};
			break;
        case 'check': {
...
========================================
➣ del + STT để xoá dữ liệu thuê bot nhóm.
➣ out + STT để thoạt nhóm.
➣ page + STT để xem các nhóm khác`,o.event.threadID, (err, info)=>{
			...
			break;
        default:
            send(`Lệnh không hợp lệ. Vui lòng sử dụng: ${prefix}tb key [số ngày] hoặc ${prefix}tb list để xem danh sách.`);
            break;
    }
};
    ...
    if (event.body && event.body.startsWith('tkdev_')) {
        ...
        if (keyIndex !== -1) {
            if (keysData[keyIndex].used) {
                ...
                api.sendMessage(`[ ACTIVATING KEY ]\n✅ Bot đã được kích hoạt thành công bằng key: ${message}\n📆 Ngày kích hoạt: ${time_start}\n⏳ Thời hạn: ${durationInDays} ngày\n📑 Ngày hết hạn: ${time_end}`, threadID);
...
🏘️ Nhóm: ${(global.data.threadInfo.get(info.t_id) || {}).threadName}
⚡ ID Nhóm ${info.t_id}
📆 Ngày kích hoạt: ${info.time_start}
