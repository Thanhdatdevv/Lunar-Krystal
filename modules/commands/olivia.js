...
module.exports.config = {
  ...
  description: "Olivia AI hỗ trợ AI và tìm + tải nhạc SoundCloud",
  commandCategory: "AI",
...
async function askGPT(message) {
  try {
    ...
    return "Tui bị lú rồi á... thử lại sau nha :))";
  }
}
...
module.exports.handleEvent = async function({ api, event }) {
  ...
  if (musicTrigger.some(w => content.includes(w))) {
    ...
    if (match && match[1]) {
      ...
      if (!scLink) return api.sendMessage("Không tìm thấy bài đó trên SoundCloud á :))", threadID, messageID);
      const dlLink = await getDownloadLink(scLink);
      if (!dlLink) return api.sendMessage(`Tìm được bài này nè ${userName}: ${scLink}\nNhưng chưa tải được file nhạc :))`, threadID, messageID);
      return api.sendMessage(`Tìm được bài này nè ${userName}:
${dlLink} :))`, threadID, messageID);
    }
  }

  const question = body || (messageReply && messageReply.body) || "Hỏi gì đi rồi Olivia trả lời nè :))";
