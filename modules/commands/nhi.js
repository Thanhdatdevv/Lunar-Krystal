...
// Bắt đầu xử lý lệnh /nhi on
if (event.body && event.body.toLowerCase() === "/nhi on") {
  const threadID = event.threadID;
  const senderID = event.senderID;

  // Nếu là bot gửi thì bỏ qua
  if (senderID == api.getCurrentUserID()) return;

  // Nếu đã bật rồi
  if (status[threadID]) {
    return api.sendMessage("Nhi đã on từ trước rùi đó~", threadID);
  }
  ...
  // Đặt biệt danh bot thành "Nhi💦"
  api.getThreadInfo(threadID, (err, info) => {
    if (err) return;
    const botID = api.getCurrentUserID();
    api.changeNickname("Nhi💦", threadID, botID);
  });

  return api.sendMessage("Đã bật Nhi rồi nè~ Mlem mlem~", threadID);
}
...
// Xử lý khi người dùng gửi /nhi off
if (event.body === "/nhi off") {
  if (!global.nhiStatus) {
    return api.sendMessage("Nhi đang tắt từ trước rồi mà.", threadID, messageID);
  }
  global.nhiStatus = false;
  return api.sendMessage("Nhi đã tắt rồi nha, hổng nói nữa đâu!", threadID, messageID);
    }
  }
};
